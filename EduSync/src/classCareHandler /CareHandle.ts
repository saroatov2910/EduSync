...
export default class CareHandler {
  ...
  static from(obj: unknown): CareHandler {
    ...
    const idNum = Number((o.handlerId as number | string | undefined) ?? (o.id as number | string | undefined) ?? 0);
  ...
  validate(): string[] {
    ...
    if (Number.isFinite(this.handlerId)) {
      ...
      if (!/^\d{4}$/.test(idStr)) errors.push("מזהה גורם חייב להיות מספר בן 4 ספרות");
    }
    if (!this.name) errors.push("שם הוא שדה חובה");
    if (!this.role || (this.role !== "מרצה" && this.role !== "מזכירות")) errors.push("תפקיד חייב להיות 'מרצה' או 'מזכירות'");
